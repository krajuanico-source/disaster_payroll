...
error_reporting(0);
	...
	$id1=$row1['empnum'];
	$sql = "SELECT  distinct e.empfname,e.empmname,e.emplname,e.empext,a.empid
					...
					<div id="subPages6" class="collapse ">
						<ul>		
							<li >
								...
								<a href="sdo.php?userid=<?php echo $emm;?>">
									<i class="pe-7s-angle-right"></i>
									<p>SDO</p>
								</a>
							</li>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="table table-bordered" style="width:100%">		   
                    <tr>  
                        <td width="30%"><label  style="font-size: 16px;font-family:Arial Narrow;">Employee ID</label></td>  
                        <td width="70%" >
                            <input class="form-control" name="empid" id="empid" disabled/>
                        </td>   
                    </tr> 
                    <tr>  
                        <td width="30%"><label  style="font-size: 16px;font-family:Arial Narrow;">SDO Status</label></td>  
                        <td width="70%" >
                            <select class="form-control selectpicker" name="sdo_status" id="sdo_status">
					 ...
					 <tr>  
                        <td width="30%"><label  style="font-size: 16px;font-family:Arial Narrow;">SDO Limit</label></td>  
                        <td width="70%" >
                        <input type="number" class="form-control" id="sdo_limit" name="sdo_limit" required>
                        </td>  
                    </tr> 	
                    <tr hidden>  
                        <td width="30%"><label  style="font-size: 16px;font-family:Arial Narrow;">SDO No.</label></td>  
                        <td width="70%" >
                            <input class="form-control" name="sdo_no" id="sdo_no"/>
                        </td>  
                    </tr> 	
                </table>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary update-status-btn">Assign SDO</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="addSdoModal" tabindex="-1" role="dialog" aria-labelledby="addSdoModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addSdoModalLabel">Add New SDO</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="addSdoForm">
                    <div class="form-group">
                        <label for="newSdoNo" style="font-size: 16px;font-family:Arial Narrow;">SDO No.</label>
                        <input type="text" class="form-control" id="newSdoNo" name="newSdoNo" required>
                    </div>
                    <div class="form-group">
                        <label for="newSdoName" style="font-size: 16px;font-family:Arial Narrow;">SDO Name</label>
							<select class="form-control selectpicker" name="sdo_id" id="sdo_id">
                                <option value="" disabled selected>----------</option>
							<?php
							$sql3 = "SELECT * FROM tbl_employment as a 
									INNER JOIN employee_info as b on a.empnum = b.empnum
									where empid<>'' and empid<>'xxx' order by emplname asc";
                    ...
                    <div class="form-group">
                        <label for="newSdoLimit" style="font-size: 16px;font-family:Arial Narrow;">Limit</label>
                        <input type="number" class="form-control" id="newSdoLimit" name="newSdoLimit" required>
                    </div>					
                    <div class="form-group">
                        <label for="newSdoStatus" style="font-size: 16px;font-family:Arial Narrow;">SDO Status</label>
                        <select class="form-control" id="newSdoStatus" name="newSdoStatus" required>
                            <option value="Active">Active</option>
                            <option value="Inactive">Inactive</option>
                        </select>
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" id="saveSdoBtn">Update SDO</button>
            </div>
        </div>
    </div>
</div>
			...
			<div class="content">
				<div class="container-fluid"> 
					...
					<table id="userTable" class="table table-bordered" style="width:100%">		   
						...
						<tbody>
							<?php
							$sql = "SELECT * FROM lib_sdo where sdo_team_leader='".$row['empid']."'";
							$result = mysqli_query($conn, $sql);
							while ($row = mysqli_fetch_assoc($result)) {
								// echo $sql2 = "SELECT * FROM tbl_employment as a 
								// 		INNER JOIN employee_info as b on a.empnum = b.empnum
								// 		where empid<>'' and empid<>'xxx' order by empid asc
								// 		empid='".$row['sdo_id']."'";


								$sql2 = "SELECT * FROM tbl_employment as a 
										INNER JOIN employee_info as b ON a.empnum = b.empnum
										WHERE empid <> '' AND empid <> 'xxx' AND empid = '".$row['sdo_id']."'
										ORDER BY empid ASC";

								$result2 = mysqli_query($con, $sql2);
								if ($row2 = mysqli_fetch_assoc($result2)) 
								echo "<tr>";
								echo "<td>" . $row['sdo_no'] . "</td>";
								echo "<td>" . $row2['emplname'] .', '. $row2['empfname'] .' '. $row2['empmname'] . "</td>";
								echo "<td align='center'>" . $row['sdo_limit']. "</td>";
								echo "<td align='center'>" . $row['sdo_status']. "</td>";
								echo "<td align='center'>" . $row['sdo_payroll_no']. "</td>";
								echo "<td><button class='btn btn-info update-btn' data-empid='" . $row2['empid'] . "' data-sdo_status='" . $row2['sdo_status'] . "' data-sdo_no='" . $row['sdo_no'] . "'>Update</button></td>";
								echo "</tr>";
							}
							?>
						</tbody>
...
$(document).ready(function() {
	$('.update-btn').click(function() {
		...
		var sdo_status = $(this).data('sdo_status');
		var sdo_no = $(this).data('sdo_no');
		var tl_id = $('#user_id').val();
        var sdo_limit = $('#sdo_limit').val();

		// Set the values of the select elements in the modal
		$('#empid').val(empid);
		$('#sdo_no').val(sdo_no);

		var selectElement = document.getElementById("sdo_status");

		// Create a new option
		var newOption = new Option("Active", "Active");

		// Add the new option as the first option
		selectElement.add(newOption, 0); // Insert at the first position		
		// Open the modal
	...
	$('.update-status-btn').click(function() {
		var empid = $('#empid').val();
		var sdo_status = $('#sdo_status').val();
		var sdo_num = $('#sdo_no').val();
		var tl_id = $('#user_id').val();
		var sdo_limit = $('#sdo_limit').val();
		
		// AJAX request to update the details in the database
		$.ajax({
			url: 'save_sdo.php',  // The PHP file that handles the update
			type: 'POST',
			data: {
				empid: empid,
				sdo_limit: sdo_limit,
				sdo_status: sdo_status,
				sdo_no: sdo_num,
				tl_id: tl_id
			},
...
$(document).ready(function() {
    ...
    $('#saveSdoBtn').click(function() {
        ...
        var newSdoName = $('#sdo_id').val();
        var newSdoStatus = $('#newSdoStatus').val();
        var newSdoLimit = $('#newSdoLimit').val();
        var tl_id = $('#user_id').val();

        // Validate the form
        if (newSdoNo === '' || newSdoName === '' || newSdoStatus === '' || newSdoLimit === '') {
        ...
        $.ajax({
            url: 'save_new_sdo.php',  // PHP file that handles saving the new SDO
            type: 'POST',
            data: {
                sdo_no: newSdoNo,
                sdo_id: newSdoName,
                sdo_status: newSdoStatus,
                sdo_limit: newSdoLimit,
                tl_id: tl_id
            },
            success: function(response) {
