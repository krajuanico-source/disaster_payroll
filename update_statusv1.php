    ...
    if ($result_bene->num_rows > 0) {
        ...
        try {
            if ($status != 'Disqualified' && $status != 'Replacement') {
                ...
                $sql_count = "SELECT COUNT(payroll_id) AS count_payroll_id FROM ect_clean_list WHERE payroll_no = ? AND status IN ('Validated','Claimed')";
                $stmt_count = $conn->prepare($sql_count);
                $stmt_count->bind_param('i', $payroll_no);
                $stmt_count->execute();
                $result_count = $stmt_count->get_result();
                $count_payroll_id = ($row = $result_count->fetch_assoc()) ? $row['count_payroll_id'] : 0;

                if ($count_payroll_id >= $sdo_limit || empty($payroll_no)) {
