...
error_reporting(0);
	...
	$sql_bene_cor = "SELECT payroll_no FROM lib_users WHERE empid = '" . $row['empid'] . "'";
		  ...
		  <div class='modal-body'>
			<table class="table table-bordered" style="width:80%">		
			  <tr>  
				...
				<td width="70%" >
				  <select class="form-control selectpicker" name="sdo" id="sdo" hidden>
					<option disabled selected>----------</option>
					<?php
					  $sqls = "SELECT * FROM lib_sdo WHERE sdo_status = 'Active'";
					  $results = mysqli_query($conn, $sqls);

					  while ($rows = mysqli_fetch_assoc($results)) {
						$sdo_empid = $rows['sdo_id'];
						$sql1s = "SELECT * FROM tbl_employee WHERE empid = '$sdo_empid'";
						$result1s = mysqli_query($con, $sql1s);
						if ($row1s = mysqli_fetch_assoc($result1s)) {
						  echo '<option value="' . $sdo_empid . '">' . strtoupper($row1s['lname'].', '.$row1s['fname'].' '.$row1s['mname']). '</option>';
						}
					  }
					?>
				  </select>
            ...
            <table id="payrollTable" style="width:100%" class="table table-bordered table-striped">
				...
				while ($result1->num_rows > 0) {
					if ($row1 = $result1->fetch_assoc()) {
						...
						// Corrected SQL query to count beneficiaries per payroll
						$payroll_no 	= $row1['payroll_no'];
						
							$sql2 = "SELECT fname, mname, lname FROM tbl_employee WHERE empid= '$sdonum'";
							$result2 = $con->query($sql2);
							$row2 = $result2->fetch_assoc();
							$sdofname =$row2['fname'].' '.$row2['mname'].' '.$row2['lname'];
						

						$sqls = "SELECT COUNT(id) as bene_count FROM tbl_bene WHERE payroll_no='$payroll_no' and status in ('Validated','Claimed')";
						$results = $conn->query($sqls);

						if ($result1 && $rows = $results->fetch_assoc()) {
							$status = $row['payroll_status']; // assume 'status' is the column name for the status
							$row_class = ($status == 'Active') ? 'success' : (($status == 'Closed') ? 'danger' : ''); // add CSS class based on status

							echo "<tr class='$row_class'>";
							echo "<td>" . $row1['payroll_no'] . "</td>";
							echo "<td>" . $program . "</td>";
							echo "<td>" . $province . "</td>";
							echo "<td>" . $date_created . "</td>";
							echo "<td>" . $amount . "</td>";
							echo "<td>" . ($city_muni ? $city_muni : 'N/A') . "</td>";
							echo "<td>" . (isset($sdofname) && $sdofname !== '' ? $sdofname : 'N/A') . "</td>";

							echo "<td>" . ($fs_name ? $fs_name : 'N/A') . "</td>";
							echo "<td align='center'>" . $rows['bene_count'] . "</td>"; // Display the count of beneficiaries
